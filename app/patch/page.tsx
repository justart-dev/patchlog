...
export default async function PatchPage() {
  ...
  return (
    <div className="relative">
      {/* Header Section */}
      <div className="relative bg-gradient-to-r from-slate-50 via-blue-50/30 to-purple-50/20 border-b border-slate-200/50">
        <div className="max-w-6xl mx-auto px-4 py-12">
          <div className="text-center space-y-4">
            <div className="inline-flex items-center space-x-2 px-4 py-2 bg-white/80 backdrop-blur-sm rounded-full border border-slate-200/50 shadow-sm">
              <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse" />
              <span className="text-sm font-medium text-slate-700">실시간 업데이트</span>
            </div>
            ...
            <p className="text-lg text-slate-600 max-w-2xl mx-auto leading-relaxed">
              매일 오후 6시(KST)에 자동으로 수집되는 최신 패치 내역을 확인하세요
            </p>
          </div>
        </div>
      </div>
        ...
        {patchLogs && patchLogs.length > 0 ? (
          <div className="space-y-8">
            <div className="flex items-center justify-between">
              <h2 className="text-2xl font-bold text-slate-900">
                최근 패치 노트 <span className="text-blue-600">({patchLogs.length}개)</span>
              </h2>
              <div className="flex items-center space-x-2 text-sm text-slate-500">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                <span>마지막 업데이트: 오늘</span>
              </div>
            </div>
          ...
          <div className="text-center py-20">
            ...
            <p className="text-slate-600 max-w-md mx-auto leading-relaxed mb-8">
              매일 자동으로 Marvel Rivals의 최신 패치 노트를 수집하여 한글로 번역합니다. 
              곧 새로운 내용을 만나보실 수 있습니다.
            </p>
