...
// Isolated Stats Component for Security & Performance
function PatternAnalysisContent({ patchLogs }: { patchLogs: PatchLog[] }) {
  const updateStats = useMemo(() => {
    if (!patchLogs || patchLogs.length === 0) {
      return { totalCount: 0, recentCount: 0, dayCounts: Array(7).fill(0), topDayCount: 0 };
    }
  ...
  if (loadError) {
    return (
      <div className="min-h-screen flex items-center justify-center p-4">
        <StatusDisplay title="오류 발생" message={loadError} variant="error" showHomeButton className="py-12" />
      </div>
    );
  }
      ...
      <div className="relative mx-auto w-full max-w-[1200px] px-6 space-y-12">
        ...
        {patchLogs.length > 0 ? (
          <>
              ...
              <article className="glass-card p-8 relative flex flex-col justify-between overflow-hidden min-h-[320px]">
                ...
                {mounted && isLoaded && isSignedIn ? (
                  ...
                  <div className="flex flex-col">
                    <div className="flex items-end justify-between h-32 gap-2 mb-4 px-2 opacity-10">
                      {[...Array(7)].map((_, i) => (
                        <div key={i} className="flex-1 bg-archive-zinc-200 dark:bg-archive-zinc-800 rounded-t-sm h-[20%]" />
                      ))}
                    </div>
                    <div className="h-3 w-32 bg-archive-zinc-200 dark:bg-archive-zinc-800 rounded opacity-10" />
                  </div>
                )}
              </article>
              ...
              <article className="glass-card p-8 flex flex-col justify-between">
                ...
                {latestVideo ? (
                  <div className="aspect-video w-full rounded-xl overflow-hidden ring-1 ring-black/5 dark:ring-white/10 shadow-2xl">
                    <iframe className="h-full w-full" src={`${latestVideo.embedUrl}?rel=0`} title={latestVideo.title} loading="lazy" allowFullScreen />
                  </div>
                ) : (
            ...
            <section className="space-y-8 animate-hero-enter [animation-delay:200ms]">
              ...
              <div className="flex justify-center pt-10">
                <button onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })} className="px-8 py-3 glass rounded-full text-xs font-black uppercase tracking-widest hover:scale-105 transition-transform">Return to Top</button>
              </div>
            </section>
          </>
        ) : (
          <div className="glass-card py-24 text-center">
            ...
            <p className="max-w-md mx-auto text-archive-zinc-500 text-sm leading-relaxed">데이터베이스에 저장된 기록이 없습니다.</p>
          </div>
        )}
      </div>
    </div>
